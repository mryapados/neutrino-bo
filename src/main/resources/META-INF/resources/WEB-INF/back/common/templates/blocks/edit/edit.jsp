...
<div class="view-object">
	<h1>
		<s:message htmlEscape="false" code="bo.edit" text="Edit : " />
						...
						<div class="row info">
							<div class="col-md-6 col-xs-12 text-left">
								<c:if test="${not empty objectName}">
									<s:message htmlEscape="false" code="bo.field.name" text="Name" /> : <span>${objectName}</span>
								</c:if>
							</div>
							<ul class="col-md-6 col-xs-12 text-right">
								...
								<li>
									<c:if test="${not empty objectLang}">
										<s:message htmlEscape="false" code="bo.field.lang" text="Lang" /> : <span class="lang-sm lang-lbl" lang="${objectLang.code}"></span>
									</c:if>
								</li>
							</ul>
						</div>
							...
							<table class="table table-striped" role="grid">
								<tbody>
									<c:forEach var="field" items="${group.value}">
										...
										<c:choose>
											<c:when test="${!field.editable && field.mandatory}">
												<form:input type="hidden" path="${field.name}"/>
											</c:when>
											<c:when test="${field.inView || field.editable}">
												<tr>
													<td class="col-md-3">
														<s:message htmlEscape="false" var="defaultMessage" code="bo.field.${field.name}" text="${field.name}" />
														<label for="${field.name}"><s:message htmlEscape="false" code="bo.${objectType}.field.${field.name}" text="${defaultMessage}" /></label>
													</td>
													<td>
